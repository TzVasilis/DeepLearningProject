...
class Atrous_Convolution(nn.Module):
    def __init__(
        ...
        This block performs atrous (dilated) convolution, followed by
        batch normalization and ReLU activation.
        Inspired by:
        https://medium.com/@r1j1nghimire/semantic-segmentation-using-
        deeplabv3-from-scratch-b1ff57a27be

        Args:
            input_channels (int): Number of input channels.
            kernel_size (int): Size of the convolutional kernel.
            pad (int): Padding added to the input.
            dilation_rate (int): Dilation rate for atrous convolution.
            output_channels (int): Number of output channels (default:
            128).
        """
...
class ASSP(nn.Module):
    def __init__(self, in_channels: int, out_channels: int) -> None:
        """
        Atrous Spatial Pyramid Pooling (ASSP) Layer. This layer
        performs atrous convolutions with different dilation rates and
        combines the results with an image pooling operation.

        Args:
            in_channels (int): Number of input channels for
            Atrous_Convolution.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Forward pass through the ASSP layer.
...
class ResNet_50(nn.Module):
    def __init__(self, output_layer: Optional[str] = None) -> None:
        """
        ResNet50 model with customizable output layer.

        Args:
            output_layer (str, optional): Name of the output layer in
            ResNet50. Default is None.
        """
...
class Deeplabv3Plus(nn.Module):
    def __init__(self, input_channels: int, output_channels: int) -> None:
        ...
        self.classifer = nn.Conv2d(256, output_channels, 1)
